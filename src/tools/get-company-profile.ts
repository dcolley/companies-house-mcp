...
export class GetCompanyProfileTool implements MCPTool {
  ...
  private formatCompanyProfile(profile: CompanyProfile) {
    const sections = [
      ...
      profile.type && profile.jurisdiction ? `**Type**: ${profile.type} (${profile.jurisdiction})` : null,
      ...
      profile.accounts ? [
        "**Accounts**:",
        this.formatAccounts(profile.accounts)
      ].join("\n") : null,
      
      // Annual return/confirmation statement
      profile.confirmation_statement || profile.annual_return ? [
        "**Annual Return/Confirmation Statement**:",
        this.formatConfirmationStatement(profile.confirmation_statement || profile.annual_return || {})
      ].join("\n") : null
    ];
