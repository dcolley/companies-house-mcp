...
describe("GetPersonsWithSignificantControlTool", () => {
  ...
  describe("execute", () => {
    ...
    it("should get PSCs successfully", async () => {
      ...
      expect(result.content[0]!.text).toContain("**Persons with Significant Control for 12345678**");
    ...
    it("should handle no PSCs found", async () => {
      mockClient.getPersonsWithSignificantControl.mockResolvedValue({ items: [], total_results: 0, start_index: 0 });

      const result = await tool.execute({ companyNumber: "12345678", limit: 25, startIndex: 0 });

      expect(result.isError).toBeUndefined();
      expect(result.content[0]!.text).toContain("No persons with significant control found for company 12345678");
    });
